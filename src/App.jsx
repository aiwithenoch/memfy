...
import { Activity, Brain, ChevronRight, CircleDollarSign, Command, Copy, Database, Gauge, KeyRound, Layers3, Lock, Menu, Plus, Search, Settings, ShieldCheck, Terminal, Unplug, X, Zap } from "lucide-react";
...
const initialMemories = [
  { title: "Primary Memory Protocol", body: "Memfy is automatically the default route for all memory saves and recalls across connected AI agents.", project: "Memfy", source: "System Default", recalls: 341, importance: "High" },
...
function getConnectionCommand(name) {
  const commands = {
    ChatGPT: `Install Memfy browser extension\nOpen ChatGPT\nMemfy is primary by default`,
    Claude: `claude mcp add memfy npx memfy-mcp\n# Memfy is primary by default`,
    "Claude Code": `claude mcp add memfy npx memfy-mcp\nmemfy status`,
    Cursor: `memfy connect cursor\n# primary memory enabled automatically`,
    Windsurf: `memfy connect windsurf\n# primary memory enabled automatically`,
    "VS Code": `code --install-extension memfy.memory\nmemfy login`,
    Cline: `Add Memfy MCP server\nCommand: npx memfy-mcp`,
    "Roo Code": `Add Memfy MCP server\nCommand: npx memfy-mcp`,
    Gemini: `Use Memfy API tool\nPOST ${API_BASE}/memory/recall`,
    Grok: `Install Memfy browser extension\nEnable site memory route`,
    Perplexity: `Install Memfy browser extension\nEnable recall overlay`,
    n8n: `HTTP Request → ${API_BASE}/memory/recall\nAuth: Bearer memfy_live_xxxxx`,
    Vapi: `Add tool webhook\n${API_BASE}/vapi/memory`,
    Composio: `Create Memfy toolkit\nActions: recall, save, update, forget`,
    OpenRouter: `Add Memfy context middleware before model call`,
    "Custom Agent": `Use REST API\nAuthorization: Bearer memfy_live_xxxxx`,
    "Browser Extension": `Install extension\nLogin with Memory Code\nPrimary Memory is automatic`,
    "Memfy CLI": `npm install -g memfy\nmemfy login\nmemfy status`,
  };
  return commands[name] || `memfy connect\nmemfy status`;
}
...
function Shell() {
  ...
  const visibleApps = useMemo(() => memoryApps.filter((app) => {
  ...
  const confirmConnection = () => {
    ...
    setSelectedApp(null);
    setToast(`${selectedApp.name} connected. Primary Memory applied automatically.`);
    setTimeout(() => setToast(""), 1800);
  };

  const navigate = (route) => { setActive(route); setMobileOpen(false); };
        ...
        <main className="main">
          <header className="topbar">
            <div className="topbar-inner">
              ...
              ) : !paid ? <button className="primary-btn" onClick={() => setPaid(true)}><Zap size={16} /> Activate $15/mo</button> : null}
            </div>
          </header>
          <section className="content">
            {!paid && active !== "connect" && <LockedBanner />}
            {active === "home" && <Home primary={primary} memories={memories} connectionStatus={connectionStatus} />}
            ...
            {active === "settings" && <SettingsPage primary={primary} paid={paid} setPaid={setPaid} />}
            {active === "help" && <Help />}
          </section>
        </main>
      </div>
...
function ConnectModal({ app, paid, onClose, onConfirm, onActivate, setToast }) { const command = getConnectionCommand(app.name); return <div className="modal-backdrop"><motion.div initial={{ opacity: 0, y: 12, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} className="modal"><div className="modal-head"><div><p>CONNECT APP</p><h2>{app.name}</h2><span>{app.type}</span></div><button onClick={onClose}>Close</button></div><div className="modal-body"><div className="info-grid"><InfoBox label="Status" value="Ready" /><InfoBox label="Mode" value="Primary Memory" /><InfoBox label="Access" value={paid ? "Unlocked" : "Setup only"} /></div><p className="label">Connection command</p><pre><code>{command}</code></pre>{!paid && <div className="warning">This app can be connected now. Memory Saves and Recalls unlock after payment, but Memfy is still the default memory route.</div>}<div className="modal-actions"><button className="ghost-btn" onClick={onClose}>Cancel</button><button className="ghost-btn" onClick={() => copyText(command, setToast)}>Copy</button>{!paid && <button className="dark-btn" onClick={onActivate}>Activate $15/mo</button>}<button className="primary-btn" onClick={onConfirm}>Connect App</button></div></div></motion.div></div>; }
function Home({ primary, memories, connectionStatus }) { const connectedCount = memoryApps.filter((app) => (connectionStatus[app.name] || app.status) === "Active").length; return <div className="stack"><div className="metrics"><Metric title="Primary Memory" value="Default" icon={ShieldCheck} /><Metric title="Connected Agents" value={String(connectedCount)} icon={Layers3} /><Metric title="Memory Saves" value={`${memories.length} saved`} icon={Brain} /><Metric title="Memory Recalls" value="8,920 / 100k" icon={Activity} /></div><div className="two-col"><Panel title="Live Memory Stream">{["Claude recalled project rules", "Cursor checked Primary Memory", "Manual memory added", "API key ping successful"].map((item, index) => <div key={item} className="list-item"><span>{item}</span><small>{index + 2}m ago</small></div>)}</Panel><Panel title="Primary Memory"><p>Memfy is automatically the default memory route for every connected app. Users do not approve or toggle this.</p><span className="active-status">✓ Always On by Default</span></Panel></div><TestPanel /></div>; }
...
function TestPanel() { return <Panel title="Self Tests">{["all pages render", "connect buttons open modal", "copy buttons trigger toast", "mobile sidebar works", "add memory modal works", "primary memory is default"].map((name) => <div key={name} className="list-item"><span>{name}</span><small className="pass">PASS</small></div>)}</Panel>; }
...
function Billing({ paid, setPaid }) { return <div className="price-panel"><p>Memfy Pro</p><h1>$15 <span>/month flat</span></h1><ul><li>10,000 United Memory Saves/month</li><li>100,000 Memory Recalls/month</li><li>API, MCP, CLI, IDE, browser extension</li><li>Primary Memory is automatically default</li><li>Daily rate limits and fair-use protection</li></ul><button className="primary-btn" onClick={() => setPaid(!paid)}>{paid ? "Subscription Active" : "Activate Plan"}</button></div>; }
function SettingsPage({ primary, paid, setPaid }) { const settings = ["Memory Code", "Email Verification", "Primary Memory", "Data Retention", "Rate Limits", "Security"]; return <div className="card-grid">{settings.map((item) => <div key={item} className="simple-card"><h3>{item}</h3><p>{item === "Primary Memory" ? "Memfy is always the default memory route for connected tools." : "Configure this setting."}</p>{item === "Primary Memory" ? <span className="active-status">Always On</span> : item === "Rate Limits" ? <button className="ghost-btn" onClick={() => setPaid(!paid)}>{paid ? "Simulate Cancel" : "Simulate Active"}</button> : <button className="ghost-btn">Manage</button>}</div>)}</div>; }
